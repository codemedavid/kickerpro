-- ============================================
-- AI AUTOMATION MONITORING - FIXED VERSION
-- Run this in Supabase SQL Editor to enable live monitoring
-- ============================================

-- Step 1: Create automation contact state tracking table
CREATE TABLE IF NOT EXISTS ai_automation_contact_states (
  ...
  max_follow_ups INTEGER DEFAULT 0,
...
-- Step 3: Create view for active automation monitoring (FIXED - removed run_24_7 reference)
...
-- Step 4: Create function to update minutes_until_send automatically
...
-- Step 5: Add trigger to auto-update timestamps
DROP TRIGGER IF EXISTS update_automation_state_timestamp ON ai_automation_contact_states;
...
-- Step 6: Create function to get stage summary for a rule
...
CREATE POLICY "Users can view their automation states" ON ai_automation_contact_states
  ...
  USING (
    EXISTS (
      ...
      WHERE ai_automation_rules.id = ai_automation_contact_states.rule_id 
      AND ai_automation_rules.user_id = auth.uid()
    )
  );

-- Allow users to insert/update their own automation states
DROP POLICY IF EXISTS "Users can manage their automation states" ON ai_automation_contact_states;
CREATE POLICY "Users can manage their automation states" ON ai_automation_contact_states
  FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM ai_automation_rules 
...
COMMENT ON FUNCTION get_automation_stage_summary(UUID) IS 'Returns stage breakdown for a specific automation rule';
...
-- SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'ai_automation_contact_states');

-- Check if view was created
-- SELECT EXISTS (SELECT 1 FROM information_schema.views WHERE table_name = 'active_automation_contacts');

-- Check if function was created
-- SELECT EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_name = 'get_automation_stage_summary');

-- Test the view (should return empty result if no active automations)
-- SELECT * FROM active_automation_contacts LIMIT 5;

-- ============================================
-- SUCCESS! 🎉
-- ============================================
-- Your AI Automation monitoring is now set up!
-- The API endpoints will now work properly.
