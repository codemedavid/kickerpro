-- Tags System for Messenger Conversations (Fixed for existing schema)
...
-- Create conversation_tags junction table (references messenger_conversations)
...
-- Conversation tags policies (using messenger_conversations table)
...
-- Insert some default tags (only if user exists)
DO $$
DECLARE
  first_user_id UUID;
BEGIN
  ...
  -- Only insert default tags if a user exists
  IF first_user_id IS NOT NULL THEN
    INSERT INTO tags (name, color, created_by) VALUES
      ('Important', '#EF4444', first_user_id),
      ('Follow Up', '#F59E0B', first_user_id),
      ('VIP', '#8B5CF6', first_user_id),
      ('Support', '#10B981', first_user_id),
      ('Sales', '#3B82F6', first_user_id)
