...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[Lead Analyze] Analyzing ${conversations.length} conversations`);
    ...
    const conversationsWithMessages = await Promise.all(
      conversations.map(async (conv) => {
        ...
        try {
          ...
          console.warn(`[Lead Analyze] Could not fetch messages for ${conv.id}`);
        }
    ...
    if (autoCreateOpportunities) {
      try {
        ...
        if (autoCreateResponse.ok) {
          ...
          console.log(`[Lead Analyze] Auto-created ${opportunitiesCreated} opportunities`);
        }
      } catch (error) {
        console.warn('[Lead Analyze] Auto-create opportunities failed:', error);
      }
    }
